module Puppet::Parser::Functions
    ...
    reference_map       = Hash(lookup_global_silent('compliance_map'))
    ...
    if hitchhiker
      ...
      @compliance_map ||= Object.const_get("PuppetX::SIMP#{Puppet[:environment]}::ComplianceMap").new(compliance_profiles, reference_map, main_config)
    end
    ...
    if main_call
      ...
      if main_config[:server_report]
        ...
        File.open(File.join(report_dir,'compliance_report.yaml'),'w') do |fh|
    ...
    # Embed a File resource that will place the YAML report on the client.
